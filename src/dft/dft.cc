...
// Libxc external library
#include <xc.h>

// gau2grid external library
#include "gau2grid.h"
...
double DFTSolver::compute_energy() {
    ...
    do {
        ...
        if (functional->is_x_hybrid()) {
            double alpha = functional->x_alpha();
            HF_ex_energy -= 0.5 * alpha * Da_->vector_dot(jk->K()[0]);
            HF_ex_energy -= 0.5 * alpha * Db_->vector_dot(jk->K()[1]);
            two_electron_energy += HF_ex_energy;
        }

        double LR_Coulomb_energy = 0.0;
        if (is_x_lrc) {
            double beta = 1.0 - functional->x_alpha();
            LR_Coulomb_energy -= 0.5 * beta * Da_->vector_dot(jk->wK()[0]);
            LR_Coulomb_energy -= 0.5 * beta * Db_->vector_dot(jk->wK()[1]);
            two_electron_energy += LR_Coulomb_energy;
        }
        ...
        if (functional->needs_xc()) {
            ...
            if (options_.get_str("FUNCTIONAL") == "SVWN") {

               std::shared_ptr<Vector> rho = (std::shared_ptr<Vector>)(new Vector(phi_points_));

               std::shared_ptr<Vector> exc = (std::shared_ptr<Vector>)(new Vector(phi_points_));
               rho->zero();
               rho->add(rho_a_);
               rho->add(rho_b_);

               double *rhop = rho->pointer();
               double *excp = exc->pointer();

               xc_func_type func;
               // xc_func_set_dens_threshold(&func,1.0e-15);
               xc_func_init(&func, XC_LDA_C_VWN_RPA, XC_UNPOLARIZED);
               xc_lda_exc(&func, phi_points_, rhop, excp);

               double ans = 0.0;
               for (int p = 0; p < phi_points_; p++) {
                   if (rhop[p] > 1.0e-20)
                      ans += grid_w_->pointer()[p] * excp[p] * rhop[p];
               }

               xc_func_end(&func);

               printf("\n\n"); 
//               printf("     EXC_SVWN3       %20.12lf\n", EX_LSDA_I(rho_a_, rho_b_));// + EC_VWN3_RPA_III());
               printf("     EXC_SVWN3       %20.12lf\n", EC_VWN3_RPA_III());
//               printf("     E_XC            %20.12lf\n", exchange_correlation_energy-EC_VWN3_RPA_III());
               printf("     E_XC            %20.12lf\n", exchange_correlation_energy-EX_LSDA_I(rho_a_,rho_b_));
               printf("     E_XC (Libxc)    %20.12lf\n", ans);

            }else if (options_.get_str("FUNCTIONAL") == "BOP") {


                     printf("\n\n"); 
                     printf("     EXC_BOP         %20.12lf\n", EX_B88_I() + EC_B88_OP());
                     printf("     E_XC            %20.12lf\n", exchange_correlation_energy);

            }else if (  options_.get_str("FUNCTIONAL") == "PBE" 
                     || options_.get_str("FUNCTIONAL") == "REVPBE") {
            
                     printf("\n\n"); 
                     printf("     EXC_PBE %20.12lf\n", EX_PBE_I() + EC_PBE_I());
                     printf("     E_XC    %20.12lf\n", exchange_correlation_energy);

            }else if (options_.get_str("FUNCTIONAL") == "WPBE") {
            
                     // double alpha = functional->x_alpha();
                     // double beta  = 1.0 - alpha;
                     // double om    = functional->x_omega();
                     printf("\n\n");
                     // printf("alpha,omega = %12.5lf %12.5lf \n",alpha,om);
                     printf("     EXC_wPBE %20.12lf\n", EX_wPBE_I() + EC_PBE_I());
                     printf("     E_XC     %20.12lf\n", exchange_correlation_energy);

            }else if (options_.get_str("FUNCTIONAL") == "BLYP") {
            
                     printf("\n\n"); 
                     printf("     EXC_BLYP %20.12lf\n", EX_B88_I() + EC_LYP_I());
                     printf("     E_XC     %20.12lf\n", exchange_correlation_energy);
             
            }else if (options_.get_str("FUNCTIONAL") == "B3LYP") {
            
                     printf("\n\n"); 
                     printf("     EXC_B3LYP %20.12lf\n", EXC_B3LYP());
                     printf("     E_XC      %20.12lf\n", exchange_correlation_energy);
            } 
        }
    ...
    // printf("\t LDA\t\t\t LSDA\t\t\t PBE\t\t RPBE\t\t\t UPBE\t\t\t EX\n");
    printf("deriv?    %i\nis_gga?   %i\nis_meta?  %i\nis_unpolarized? %i",deriv_, is_gga_, is_meta_, is_unpolarized_);
    outfile->Printf("\n");
...
void DFTSolver::BuildRho(std::shared_ptr<Matrix> Da, std::shared_ptr<Matrix> Db) {
    ...
    if ( is_gga_ || is_meta_ ) {
        ...
        sigma_ = (std::shared_ptr<Vector>)(new Vector(phi_points_));
        ...
        for (int p = 0; p < phi_points_; p++) {
            ...
            sigma_p[p] = sigma_aap[p] + 2.0 * sigma_abp[p] + sigma_bbp[p]; 
